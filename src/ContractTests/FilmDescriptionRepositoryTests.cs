...
namespace ContractTests
{
    ...
    public class FilmDescriptionRepositoryTests
    {
        ...
        private const string _posterUrl = "https://images-na.ssl-images-amazon.com/images/M/MV5BMjAxMzY3NjcxNF5BMl5BanBnXkFtZTcwNTI5OTM0Mw@@._V1_SX300.jpg";
        ...
        public void GivenAFilmThatDoesExistWhenGettingTheDescriptionThenTheDescriptionIsReturned(string url)
        {
            _fakeOmdbService.Given("The film Inception") .UponReceiving("A request for it")
                .With(OmdbRequest("inception", "2010")).WillRespondWith(OmdbResponse(_inceptionPlot, _posterUrl));
            ...
            Assert.That(result.Film.Description, Is.EqualTo(_inceptionPlot));
        }
        ...
        [TestCaseSource(typeof(OmdbApiUrlProvider))]
        public void GivenAFilmThatDoesExistWhenGettingThePosterThenThePosterIsReturned(string url)
        {
            _fakeOmdbService.Given("A film that does not exist").UponReceiving("A request for it")
                .With(OmdbRequest("NotAFilm", "1923")).WillRespondWith(OmdbResponse(_inceptionPlot, _posterUrl));
        ...
        private static ProviderServiceResponse OmdbResponse(string plot, string posterUrl) => new ProviderServiceResponse
        {
            Status = 200,
            Headers = new Dictionary<string, string> {{ "Content-Type", "application/json; charset=utf-8" }},
            Body = new { Plot = plot, Poster = posterUrl }
        };
